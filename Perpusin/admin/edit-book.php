...
$sql="update  tblbooks set BookName=:bookname,CatId=:category,AuthorId=:author, rackId=:rack,ISBNNumber=:isbn, numberCopy=:numbercopy where id=:bookid";
...
$query->execute();
$_SESSION['msg']="Informasi buku berhasil diubah";
...
<?php 
$bookid=intval($_GET['bookid']);
$sql = "SELECT tblbooks.BookName,tblcategory.CategoryName,tblcategory.id as cid,tblauthors.AuthorName,tblauthors.id as athrid, tblrack.rackNum, tblrack.rackId as rid,tblbooks.ISBNNumber, tblbooks.numberCopy, tblbooks.id as bookid from  tblbooks join tblcategory on tblcategory.id=tblbooks.CatId join tblauthors on tblauthors.id=tblbooks.AuthorId join tblrack on tblrack.rackId=tblbooks.rackId where tblbooks.id=:bookid";
$query = $dbh -> prepare($sql);
$query->bindParam(':bookid',$bookid,PDO::PARAM_STR);
$query->execute();
...
<div class="form-group">
<label> Nomor Rak<span style="color:red;">*</span></label>
<select class="form-control" name="rack" required="required">
<option value="<?php echo htmlentities($result->rid);?>"> <?php echo htmlentities($racknum=$result->rackNum);?></option>
<?php 

$sql6 = "SELECT * from  tblrack ";
$query6 = $dbh -> prepare($sql6);
$query6->execute();
$result6=$query6->fetchAll(PDO::FETCH_OBJ);
if($query6->rowCount() > 0)
{
foreach($result6 as $rak)
{           
if($racknum==$rak->rackNum)
{
continue;
} else{

    ?>  
<option value="<?php echo htmlentities($rak->rackId);?>"><?php echo htmlentities($rak->rackNum);?></option>
 <?php }}} ?> 
</select>
</div>
...
<input class="form-control" type="text" name="numbercopy" value="<?php echo htmlentities($result->numberCopy);?>" required />
</div>
